...
#define OPT_LONGHELP 263
    ...
    /* open the input */
    {
        ...
        if ( b_y4m )
        {
            printf("Using read frame ym4 here!\n");
...
#ifdef HAVE_PTHREAD
    printf("Have pthread here!\n");
    printf("Number of i threads here %d !\n", param->i_threads);
    printf("Number of CPU processors here %d !\n", x264_cpu_num_processors());
        ...
        || (param->i_threads == 0 && x264_cpu_num_processors() > 1) )
    {
        ...
        {
            printf("Using read frame thread here!\n");
